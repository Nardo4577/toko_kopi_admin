...
<html lang="en">
    <head>
        ...
        <title>Portal Admin - Kelola Menu</title>
        ...
        <style>
            ...
            .upload-title { font-size: 12px; color: #555; margin-bottom: 5px; font-weight: bold; }
        </style>
    </head>
    <body>
                                    ...
                                    <div class="container">
                                        ...
                                        <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                                            <ul class="nav navbar-nav navbar-right navBar">
                                                ...
                                                <li class="nav-item active"><a href="<?= base_url('admin/stok') ?>" class="nav-link text-uppercase font-weight-bold" style="color: #fff; text-shadow: 1px 1px 2px rgba(0,0,0,0.8);">Kelola Menu</a></li>
        ...
        <div class="main-content">
            <div class="banner" style="background-image: url('https://images.unsplash.com/photo-1559925393-8be0ec4767c8?auto=format&fit=crop&w=1900&q=80'); background-position: center; background-attachment: fixed; position: relative;">
                ...
                <div class="container text-center" style="position: relative; z-index: 2; padding-top: 180px; padding-bottom: 100px;">
                    <h2 style="color: white; font-family: 'Righteous', cursive; font-size: 45px; margin-bottom: 10px;">Manajemen Menu</h2>
                    <h4 style="color: #f1c40f; font-family: 'Poppins', sans-serif;">Pantau dan kelola daftar menu di kedai.</h4>
                </div>
            </div>
                            ...
                            <div class="card admin-card">
                                <div class="card-body">
                                    <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #f0e9e1; padding-bottom: 15px; margin-bottom: 20px; flex-wrap: wrap; gap: 15px;">
                                        <h4 style="font-family: 'Righteous', cursive; color: #3e2723; margin: 0; font-size: 22px;">Daftar Menu Saat Ini</h4>
    
                                        <div style="display: flex; gap: 10px; align-items: center;">
        
                                            <form action="<?= base_url('admin/stok') ?>" method="GET" style="margin: 0; display: flex; gap: 10px;">
                                                <input type="text" id="searchMenu" name="keyword" class="custom-input" placeholder="Cari nama menu..." value="<?= isset($keyword_terpilih) ? htmlspecialchars($keyword_terpilih) : '' ?>" style="margin-bottom: 0; padding: 7px 15px; width: 200px;" autocomplete="off">
                                                
                                                <select name="kategori" class="custom-input" style="margin-bottom: 0; padding: 7px 15px; cursor: pointer; width: auto;" onchange="this.form.submit()">
                                                    <option value="">-- Semua Kategori --</option>
                                                    <option value="Minuman" <?= (isset($kategori_terpilih) && $kategori_terpilih == 'Minuman') ? 'selected' : '' ?>>Minuman</option>
                                                    <option value="Makanan" <?= (isset($kategori_terpilih) && $kategori_terpilih == 'Makanan') ? 'selected' : '' ?>>Makanan</option>
                                                    <option value="Cemilan" <?= (isset($kategori_terpilih) && $kategori_terpilih == 'Cemilan') ? 'selected' : '' ?>>Cemilan</option>
                                                </select>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-striped table-hover table-laporan">
                                            ...
                                            <tbody>
                                                <?php $no = 1; foreach ($stokBarang as $row): ?>
                                                    <tr>
                                                        <th scope="row">
                                                            <?= $no++ ?>
                                                        </th>
                                                        <td>
                                                            <?php 
                                                                if (empty($row['foto_menu'])) {
                                                                    $foto_view = 'https://via.placeholder.com/60x60?text=No+Image';
                                                                } elseif (strpos($row['foto_menu'], 'http') === 0) {
                                                                    $foto_view = $row['foto_menu']; // Jika diawali HTTP, ambil langsung link URL
                                                                } else {
                                                                    $foto_view = base_url('img/menu/' . $row['foto_menu']); // Jika tidak, panggil folder internal
                                                                }
                                                            ?>
                                                            <img src="<?= $foto_view ?>" alt="<?= htmlspecialchars($row['nama_menu']) ?>" style="width: 60px; height: 60px; object-fit: cover; border-radius: 8px; border: 1px solid #ddd;" onerror="this.onerror=null;this.src='https://placehold.co/60x60/eaddd3/3e2723?text=Err';">
                                                        </td>
                                                        ...
                                                        <td class="text-center">
                                                            <?php if(isset($row['status_ketersediaan']) && $row['status_ketersediaan'] == 'Habis'): ?>
                                                                <span style="background-color: #e74c3c; color: white; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: bold;">Habis</span>
                                                            <?php else: ?>
                                                                <span style="background-color: #2ecc71; color: white; padding: 4px 10px; border-radius: 12px; font-size: 11px; font-weight: bold;">Tersedia</span>
                                                            <?php endif; ?>
                                                            
                                                            <?php if(isset($row['is_bestseller']) && $row['is_bestseller'] == 1): ?>
                                                                <br><span style="color: #f39c12; font-size: 11px; font-weight: bold; display: inline-block; margin-top: 5px;"><i class="fa fa-star"></i> Andalan</span>
                                                            <?php endif; ?>
                                                        </td>

                                                        <td class="text-center">
                                                            <button type="button" class="btn btn-sm btn-warning text-white" title="Edit" data-toggle="modal" data-target="#modalEdit<?= $row['id_menu'] ?>">
                                                                <i class="fa fa-pencil"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-sm btn-danger" title="Hapus" data-toggle="modal" data-target="#modalHapus<?= $row['id_menu'] ?>">
                                                                <i class="fa fa-trash"></i>
                                                            </button>
                                                        </td>
                                                    </tr>

                                                    <div class="modal fade" id="modalEdit<?= $row['id_menu'] ?>" tabindex="-1" role="dialog">
                                                        <div class="modal-dialog custom-modal-width" role="document">
                                                            <div class="modal-content" style="border-radius: 15px; overflow: hidden; border: none; box-shadow: 0 15px 40px rgba(0,0,0,0.2);">
                                                                <div class="custom-modal-header">
                                                                    ...
                                                                    <h4 class="custom-modal-title"><i class="fa fa-edit"></i> Edit Menu: <?= htmlspecialchars($row['nama_menu']) ?></h4>
                                                                </div>
                                                                <form action="<?= base_url('admin/update_stok') ?>" method="POST" enctype="multipart/form-data">
                                                                    <div class="custom-modal-body">
                                                                        <input type="hidden" name="id_menu" value="<?= $row['id_menu'] ?>">

                                                                        <label class="custom-form-label">Nama Menu</label>
                                                                        <input type="text" class="custom-input" name="nama_menu" value="<?= htmlspecialchars($row['nama_menu']) ?>" required>

                                                                        <label class="custom-form-label">Kategori</label>
                                                                        <select class="custom-input" name="kategori" required>
                                                                            ...
                                                                            <option value="Cemilan" <?= $row['kategori'] == 'Cemilan' ? 'selected' : '' ?>>Cemilan</option>
                                                                        </select>

                                                                        <label class="custom-form-label">Harga Satuan</label>
                                                                        <div class="input-group" style="margin-bottom: 15px;">
                                                                            <span class="input-group-addon" style="background-color: #f0e9e1; border-color: #d7ccc8;">Rp</span>
                                                                            <input type="number" class="custom-input" name="harga" value="<?= $row['harga'] ?>" style="margin-bottom: 0;" required>
                                                                        </div>

                                                                        <label class="custom-form-label">Status Ketersediaan</label>
                                                                        <select class="custom-input" name="status_ketersediaan" required>
                                                                            <option value="Tersedia" <?= (isset($row['status_ketersediaan']) && $row['status_ketersediaan'] == 'Tersedia') ? 'selected' : '' ?>>Tersedia</option>
                                                                            <option value="Habis" <?= (isset($row['status_ketersediaan']) && $row['status_ketersediaan'] == 'Habis') ? 'selected' : '' ?>>Habis</option>
                                                                        </select>

                                                                        <div style="background-color: #fff9e6; border: 1px solid #f1c40f; padding: 10px 15px; border-radius: 8px; margin-bottom: 15px; display: flex; align-items: center;">
                                                                            <input type="checkbox" name="is_bestseller" value="1" style="margin: 0 10px 0 0; cursor: pointer; transform: scale(1.3);" <?= (isset($row['is_bestseller']) && $row['is_bestseller'] == 1) ? 'checked' : '' ?>>
                                                                            <label style="margin: 0; font-family: 'Poppins', sans-serif; font-size: 13px; color: #d35400; font-weight: 600; cursor: pointer;">Jadikan Menu Andalan (Best Seller)</label>
                                                                        </div>

                                                                        <label class="custom-form-label">Update Foto Menu (Pilih Salah Satu Opsi)</label>
                                                                        <div class="upload-box">
                                                                            <p class="upload-title">Opsi 1: Upload File dari Laptop/Komputer</p>
                                                                            <input type="file" class="custom-input" name="foto_menu" accept="image/*" style="padding: 7px; margin-bottom: 15px;">
                                                                            
                                                                            <p class="upload-title">Opsi 2: Link URL dari Internet</p>
                                                                            <?php 
                                                                                $is_url = (strpos($row['foto_menu'], 'http') === 0); 
                                                                                $nilai_url = $is_url ? $row['foto_menu'] : '';
                                                                            ?>
                                                                            <input type="url" class="custom-input" name="url_foto" value="<?= $nilai_url ?>" placeholder="https://contoh.com/gambar.jpg" style="margin-bottom: 0;">
                                                                        </div>
                                                                        <small style="color: #888; font-size: 11px;">*Kosongkan keduanya jika tidak ingin mengubah foto</small>

                                                                        <button type="submit" class="btn btn-action-kopi" style="width: 100%; border-radius: 8px; margin-top: 15px;">Simpan Perubahan</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div class="modal fade" id="modalHapus<?= $row['id_menu'] ?>" tabindex="-1" role="dialog">
                                                        <div class="modal-dialog custom-modal-width" role="document">
                                                            <div class="modal-content" style="border-radius: 15px; overflow: hidden; border: none; box-shadow: 0 15px 40px rgba(0,0,0,0.2);">
                                                                ...
                                                                <form action="<?= base_url('admin/hapus_stok') ?>" method="POST">
                                                                    <div class="custom-modal-body text-center">
                                                                        <input type="hidden" name="id_menu" value="<?= $row['id_menu'] ?>">
                                                                        <p style="font-family: 'Poppins', sans-serif; color: #555;">Apakah Anda yakin ingin menghapus <strong><?= htmlspecialchars($row['nama_menu']) ?></strong> dari daftar menu?</p>
        ...
        <div class="modal fade" id="modalTambah" tabindex="-1" role="dialog">
            <div class="modal-dialog custom-modal-width" role="document">
                <div class="modal-content" style="border-radius: 15px; overflow: hidden; border: none; box-shadow: 0 15px 40px rgba(0,0,0,0.2);">
                    <div class="custom-modal-header">
                        <button type="button" class="close" data-dismiss="modal" style="color: #fff; opacity: 0.8;"><span aria-hidden="true">&times;</span></button>
                        <h4 class="custom-modal-title"><i class="fa fa-plus-circle"></i> Tambah Menu Baru</h4>
                    </div>
                    <form action="<?= base_url('admin/simpan_stok') ?>" method="POST" enctype="multipart/form-data">
                        <div class="custom-modal-body">

                            <label class="custom-form-label">Nama Menu</label>
                            <input type="text" class="custom-input" name="nama_menu" placeholder="Masukkan nama menu" required>
                            ...
                            <div class="input-group" style="margin-bottom: 15px;">
                                <span class="input-group-addon" style="background-color: #f0e9e1; border-color: #d7ccc8;">Rp</span>
                                <input type="number" class="custom-input" name="harga" placeholder="10000" style="margin-bottom: 0;" required>
                            </div>
                            ...
                            <div style="background-color: #fff9e6; border: 1px solid #f1c40f; padding: 10px 15px; border-radius: 8px; margin-bottom: 15px; display: flex; align-items: center;">
                                <input type="checkbox" name="is_bestseller" value="1" style="margin: 0 10px 0 0; cursor: pointer; transform: scale(1.3);">
                                <label style="margin: 0; font-family: 'Poppins', sans-serif; font-size: 13px; color: #d35400; font-weight: 600; cursor: pointer;">Jadikan Menu Andalan (Best Seller)</label>
                            </div>

                            <label class="custom-form-label">Foto Menu (Pilih Salah Satu Opsi)</label>
                            <div class="upload-box">
                                <p class="upload-title">Opsi 1: Upload File dari Laptop/Komputer</p>
                                <input type="file" class="custom-input" name="foto_menu" accept="image/*" style="padding: 7px; margin-bottom: 15px;">
                                
                                <p class="upload-title">Opsi 2: Link URL dari Internet</p>
                                <input type="url" class="custom-input" name="url_foto" placeholder="https://contoh.com/gambar.jpg" style="margin-bottom: 0;">
                            </div>

                            <button type="submit" class="btn btn-action-kopi" style="width: 100%; border-radius: 8px; margin-top: 10px;">Simpan Data</button>
            ...
            $('#searchMenu').on('input', function() {
                ...
                if (keyword.length >= 3) {
                    $('.table-laporan tbody tr').each(function() {
                        var namaMenu = $(this).find('td:eq(1)').text().toLowerCase();
                        
                        if (namaMenu.indexOf(keyword) > -1) {
                            $(this).show(); 
                        } else {
                            $(this).hide(); 
                        }
